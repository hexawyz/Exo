...
namespace AnyLayout.RawInput
{
	...
	{
		public static readonly Guid HidDeviceInterfaceClassGuid = new Guid("4D1E55B2-F16F-11CF-88CB-001111000030");
...
#pragma warning restore CS0169, IDE0051, RCS1213
		...
		public static string SetupDiGetDeviceInterfaceDetail(SafeDeviceInfoListHandle handle, ref NativeMethods.DeviceInterfaceData interfaceData)
		{
			...
			try
			{
				// The buffer represents a variable length struct (SP_DEVICE_INTERFACE_DETAIL_DATA) whith only one fixed length field:
				// DWORD cbSize;
				// CHAR DevicePath[ANYSIZE_ARRAY];
				ref uint size = ref Unsafe.As<byte, uint>(ref buffer[0]);
				size = IntPtr.Size == 4 ? 6 : 8; // sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA) in C accounts for the padding between cbSize and DevicePath.
