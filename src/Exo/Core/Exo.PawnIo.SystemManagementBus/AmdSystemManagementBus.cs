...
public sealed class AmdSystemManagementBus : ISystemManagementBus, IMotherboardSystemManagementBusFeature
{
	private const uint ErrorNoSuchDevice = 0x800701B1;

	private readonly PawnIo _pawnIo;

	public AmdSystemManagementBus()
	{
		var pawnIo = new PawnIo();
		try
		{
			pawnIo.LoadKnownModule(PawnIoKnownModule.SmbusPIIX4);
		}
		catch
		{
			pawnIo.Dispose();
	...
	ValueTask ISystemManagementBus.QuickWriteAsync(byte address)
	{
		try
		{
			_pawnIo.Execute("ioctl_piix4_write_quick\0"u8, [address, 0], []);
		}
	...
	ValueTask ISystemManagementBus.QuickReadAsync(byte address)
	{
		try
		{
			_pawnIo.Execute("ioctl_piix4_write_quick\0"u8, [address, 1], []);
		}
		catch (COMException ex) when ((uint)ex.ErrorCode == ErrorNoSuchDevice)
		{
			throw new SystemManagementBusDeviceNotFoundException(address);
	...
	ValueTask ISystemManagementBus.SendByteAsync(byte address, byte value)
	{
		try
		{
			_pawnIo.Execute("ioctl_piix4_write_byte\0"u8, [address, value], []);
		}
		catch (COMException ex) when ((uint)ex.ErrorCode == ErrorNoSuchDevice)
		{
			throw new SystemManagementBusDeviceNotFoundException(address);
	...
	ValueTask<byte> ISystemManagementBus.ReceiveByteAsync(byte address)
	{
		try
		{
			...
			buffer[0] = address;
			_pawnIo.Execute("ioctl_piix4_read_byte\0"u8, buffer[..1], buffer[1..]);
	...
	ValueTask ISystemManagementBus.WriteByteAsync(byte address, byte command, byte value)
	{
		try
		{
			_pawnIo.Execute("ioctl_piix4_write_byte_data\0"u8, [address, command, value], []);
		}
		catch (COMException ex) when ((uint)ex.ErrorCode == ErrorNoSuchDevice)
		{
			throw new SystemManagementBusDeviceNotFoundException(address);
	...
	ValueTask<byte> ISystemManagementBus.ReadByteAsync(byte address, byte command)
	{
		try
		{
			...
			buffer[0] = address;
			buffer[1] = command;
			_pawnIo.Execute("ioctl_piix4_read_byte_data\0"u8, buffer[..2], buffer[2..]);
	...
	ValueTask ISystemManagementBus.WriteWordAsync(byte address, byte command, ushort value)
	{
		try
		{
			_pawnIo.Execute("ioctl_piix4_write_word_data\0"u8, [address, command, value], []);
		}
		catch (COMException ex) when ((uint)ex.ErrorCode == ErrorNoSuchDevice)
		{
			throw new SystemManagementBusDeviceNotFoundException(address);
	...
	ValueTask<ushort> ISystemManagementBus.ReadWordAsync(byte address, byte command)
	{
		try
		{
			...
			buffer[0] = address;
			buffer[1] = command;
			_pawnIo.Execute("ioctl_piix4_read_word_data\0"u8, buffer[..2], buffer[2..]);
	...
	ValueTask ISystemManagementBus.WriteBlockAsync(byte address, byte command, ReadOnlyMemory<byte> data)
	{
		try
		{
			...
			_pawnIo.Execute("ioctl_piix4_write_block_data\0"u8, buffer, []);
			return ValueTask.CompletedTask;
		}
		catch (COMException ex) when ((uint)ex.ErrorCode == ErrorNoSuchDevice)
		{
	...
	ValueTask<byte[]> ISystemManagementBus.ReadBlockAsync(byte address, byte command)
	{
		try
		{
			...
			buffer[0] = address;
			buffer[1] = command;
			_pawnIo.Execute("ioctl_piix4_read_block_data\0"u8, buffer[..2], buffer[2..]);
