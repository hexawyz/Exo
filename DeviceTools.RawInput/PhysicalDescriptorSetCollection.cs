...
namespace DeviceTools.RawInput
{
    public readonly struct PhysicalDescriptorSetCollection : IReadOnlyList<PhysicalDescriptorSet>, IList<PhysicalDescriptorSet>, IEquatable<PhysicalDescriptorSetCollection>
    {
        ...
        internal PhysicalDescriptorSetCollection(ReadOnlySpan<byte> data, bool revalidateData)
        {
            ...
            Data = data.Slice(0, length).ToArray();

            if (revalidateData && ValidateDataLength(Data) != length)
                throw new InvalidDataException("Data has been modified during construction.");
        }

        private PhysicalDescriptorSetCollection(byte[] data) => Data = data;
        ...
        internal int PhysicalDescriptorCount => Unsafe.ReadUnaligned<ushort>(ref Data[1]);
        ...
        public int IndexOf(PhysicalDescriptorSet item)
            => Data == item.Collection.Data ? (item.StartIndex - 3) / (1 + PhysicalDescriptorCount * 2) : -1;

        public bool Contains(PhysicalDescriptorSet item) => Data == item.Collection.Data;
        ...
        public bool Equals(PhysicalDescriptorSetCollection other) => Data == other.Data;
        public override int GetHashCode() => -301143667 + Data.GetHashCode();
